...
async function authOnSignedIn(user) {
  ...
  if (error) { console.error('[WT] Load error:', error); }
  ...
  // Ensure profile row exists (for DB foreign key)
  await supa.from('profiles').upsert({ code: _wtCode }, { onConflict: 'code' });
