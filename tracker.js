...
async function authInit() {
  ...
  supa.auth.onAuthStateChange(async (event, session) => {
    if (event === 'SIGNED_IN' && session?.user && !_currentUser) {
      ...
      document.getElementById('wt-auth-panel').style.display = 'block';
      authSwitchTab('login');
      authSetMsg('login', 'Enter your new password below.', 'ok');
    }
  });
}
...
function wtDetectTodayDay() {
  // Use local date (not UTC) to match what user entered in the form
  const now = new Date();
  const today = now.getFullYear() + '-' +
    String(now.getMonth()+1).padStart(2,'0') + '-' +
    String(now.getDate()).padStart(2,'0');
  const todaySets = _wtCache.filter(s => s.date === today);
  if (!todaySets.length) return null;
  const todayExNames = [...new Set(todaySets.map(s => s.exercise))];
  let bestDay = null, bestMatch = 0;
  for (const [day, exList] of Object.entries(DAY_EXERCISES)) {
    const match = todayExNames.filter(e => exList.includes(e)).length;
    if (match > bestMatch) { bestMatch = match; bestDay = day; }
  }
  return bestDay;
}

// Show full day summary — today's sets + comparison vs last time same day
function wtShowDaySummary(day) {
  ...
  const todaySets = allSets.filter(s => s.date === today);

  // Find last date this day was done (before today)
  const prevDates = [...new Set(
    allSets
      .filter(s => s.date < today && exList.includes(s.exercise))
      .map(s => s.date)
  )].sort().reverse();
  const lastDate = prevDates[0] || null;
  const lastSets = lastDate ? allSets.filter(s => s.date === lastDate) : [];

  // Check if today has any data for this day
  ...
  body.innerHTML = `
    <div class="wt-history">
      <div class="wt-history-head" style="flex-wrap:wrap;gap:8px;">
        <div>
          <div class="wt-history-title">DAY ${day} — ${wtFmtDate(today)}</div>
